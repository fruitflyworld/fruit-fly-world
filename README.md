...
| --- | --- | --- |
| 00 | **The lineage game** | [`/play`](https://fruitfly.world/play) — a free browser roguelite with a seeded, reproducible world and a **selectable brain**: drive it yourself (WASD), keep the genes auto-pilot, plug in FFW-CX/0.1 (a 24-neuron connectome-inspired spiking circuit), or hand the fly to the judgment layer (free local heuristic, or a pinned System One model via the `/api/jev` proxy). The GF brainstem still owns the escape jump. Every decision is sealed with a content hash and downloadable via `FlyLabAPI.getDecisionLog()` |
