  ...
  return <main className="essayPage">
    ...
    <article className="essayBody">
      ...
      <section>
        ...
        <p>Every decision from every brain — signals in, distribution out, behavior, confidence, danger read — is sealed with a content hash and written to a log you can download at the end of a generation (<code>flyline-log/1</code>). The brain chooses; the brainstem jumps. That division is deliberate: high-level decisions are pluggable, the reflex that keeps you alive is not.</p>
      </section>
      ...
      <section>
        ...
        <p>Getting there was a lesson in how many ways &quot;deterministic&quot; leaks. Our leaks, all found and fixed in the exam room: a novelty-memory set that survived between runs, the rival&apos;s traits living on the wrong object, predator residue — and the subtlest one, the escape neuron&apos;s <b>membrane potential</b> carrying over between runs, so run B&apos;s reflex fired on a different frame than run A&apos;s. A survival exam you cannot re-run is not an exam. Now it re-runs.</p>
        <p>Two brains, same seed, same paper: on seed 42 the judgment layer finished with 5 eggs, the 24-neuron circuit with 2. That is one seed, not a theorem — it is the first row of what this system is for: <b>comparable, replayable survival under identical pressure.</b></p>
      </section>
